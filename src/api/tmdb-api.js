...
export const getCast = (args) => {
  ...
  console.log(id);
  return fetch(
    `https://api.themoviedb.org/3/movie/{id}/credits?api_key=${process.env.REACT_APP_TMDB_KEY}>&language=en-U`
  )
    .then((response) => {
      if (!response.ok) {
        throw new Error(response.json().message);
...
export const getActor = (args) => {
  ...
  console.log(id);
  return fetch(
    `https://api.themoviedb.org/3/person/${id}?api_key=${process.env.REACT_APP_TMDB_KEY}&language=en-US&include_adult=false&include_video=false&page=1`
  )
    .then((response) => {
      if (!response.ok) {
        throw new Error(response.json().message);
